import streamlit as st

st.header("Rishi Kumra: Recent Projects")

st.write("""
1. **Natural Language Processing on 10-K filings reports to forecast returns**
   - Streamlit: [nlp10kfilingsrk.streamlit.app](https://nlp10kfilingsrk.streamlit.app/)
   - GitHub: [github.com/quantrkucb/NLP_10K_Filings](https://github.com/quantrkucb/NLP_10K_Filings)

2. **Multi-asset return prediction with feature importance**
   - GitHub (Paper + Slides): https://github.com/quantrkucb/MultiAssetReturnPrediction 

3. **Tail-Risk hedging with financial derivatives (Bank of America)**
   - Streamlit: [nlp10kfilingsrk.streamlit.app](https://nlp10kfilingsrk.streamlit.app/)
   - GitHub: [github.com/quantrkucb/NLP_10K_Filings](https://github.com/quantrkucb/NLP_10K_Filings)
   - Paper: [Link to Paper]()

4.  **Order Book Visualizer**
   - Streamlit: https://orderbookvisualiserrk.streamlit.app/ 
   - GitHub: https://github.com/quantrkucb/OrderBookVisualiser 

5. **Portfolio Optimization: Analyzing and comparing models**
   - Streamlit: [nlp10kfilingsrk.streamlit.app](https://nlp10kfilingsrk.streamlit.app/)
   - GitHub: [github.com/quantrkucb/NLP_10K_Filings](https://github.com/quantrkucb/NLP_10K_Filings)

6. **[Cal Hackathon] Robo-advisor for retail investors**
   - GitHub: [github.com/quantrkucb/CalHacksStockly](https://github.com/quantrkucb/CalHacksStockly/) 
   - Website: [devpost.com/software/stockly](https://devpost.com/software/stockly)

""")
